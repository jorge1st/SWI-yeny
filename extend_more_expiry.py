import sqlite3, datetime, random, os, sys, re
...
try:
    ...
    print("ERROR querying productos:", e); conn.close(); sys.exit(1)
rows = cur.fetchall()
...
for id_, f in rows:
    ...
    if not parsed:
        continue
    # If expiry is within threshold (soon to expire) or already expired, add 30-60 days
    if parsed <= threshold:
